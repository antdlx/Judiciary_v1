...
class TextRNN:
    def __init__(self,num_classes, learning_rate, batch_size, decay_steps, decay_rate,sequence_length,
                 vocab_size,embed_size,hidden_size,is_training,initializer=tf.random_normal_initializer(stddev=0.1)):
        ...
        #tf.assign(old,new)更新变量内容，相当于执行epoch_step += 1
        self.epoch_increment=tf.assign(self.epoch_step,tf.add(self.epoch_step,tf.constant(1)))
        #decay_steps和decay_rate一起控制learning_rate随训练次数增加而缩小
        self.decay_steps, self.decay_rate = decay_steps, decay_rate

        #初始化权重和embedding_table
...
def test():
    ...
    hidden_size = 128
    textRNN=TextRNN(num_classes, learning_rate, batch_size, decay_steps, decay_rate,sequence_length,vocab_size,embed_size,hidden_size,is_training)
