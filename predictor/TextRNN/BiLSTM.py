...
def main():
    ...
    train_dataset = tf.data.Dataset.from_tensor_slices((train_fact_ids, train_label_ids))
    train_dataset = train_dataset.shuffle(10000).batch(train_batch_size)

    dev_dataset = tf.data.Dataset.from_tensor_slices((valid_fact_ids, valid_label_ids))
    dev_dataset = dev_dataset.shuffle(10000).batch(dev_batch_size)

    test_dataset = tf.data.Dataset.from_tensor_slices((test_fact_ids, test_label_ids))
